...
fn main() {
    ...
    HttpServer::new(move || {
        ...
        .service(web::resource("/phaser/v1/scans/{scan_id}")
            .route(web::delete().data(api::json_default_config()).to_async(phaserv1::scans::scan::delete))
        )
        .service(web::resource("/phaser/v1/scans/{scan_id}/queue")
            .route(web::post().data(api::json_default_config()).to_async(phaserv1::scans::scan::queue::post))
        )
        .service(web::resource("/phaser/v1/scans/{scan_id}/cancel")
            .route(web::post().data(api::json_default_config()).to_async(phaserv1::scans::scan::cancel::post))
        )
        .service(web::resource("/phaser/v1/scans/{scan_id}/reports/{report_id}/complete")
            .route(web::post().data(api::json_default_config()).to_async(phaserv1::scans::scan::reports::report::complete::post))
        )
    })
    .backlog(8192)
