...
pub fn post((download_id, download_data, req): (Path<(uuid::Uuid)>, Json<models::FailDownloadBody>, HttpRequest<api::State>)) -> FutureResponse<HttpResponse> {
    ...
    return state.db
    .send(controllers::FailDownload{
        download_id: download_id.into_inner(),
        error: download_data.error.clone(),
