...
impl Message for UnarchiveNote {
    type Result = Result<(), KernelError>;
}

impl Handler<UnarchiveNote> for DbActor {
    type Result = Result<(), KernelError>;

    fn handle(&mut self, msg: UnarchiveNote, _: &mut Self::Context) -> Self::Result {
        use diesel::prelude::*;
        ...
        return Ok(conn.transaction::<_, KernelError, _>(|| {
            ...
            let unarchive_cmd = note::Unarchive { metadata };
            ...
            let (note_to_update, event, _) =
                eventsourcing::execute(&conn, note_to_update, &unarchive_cmd)?;
