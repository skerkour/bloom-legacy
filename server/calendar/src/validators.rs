...
pub fn event_dates(start_at: chrono::DateTime<chrono::Utc>, end_at: chrono::DateTime<chrono::Utc>) -> Result<(), KernelError> {
    if end_at < start_at {
...
pub fn event_description(description: &str) -> Result<(), KernelError> {
    if description.len() > 2048 {
        return Err(KernelError::Validation("description is too long".to_string()));
    }
