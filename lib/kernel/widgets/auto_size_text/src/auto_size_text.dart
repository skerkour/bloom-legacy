 ...
 void initState() {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints size) {
      ...
      final int maxLines = widget.maxLines ?? defaultTextStyle.maxLines;

      _sanityCheck(style, maxLines);

      final List<dynamic> result = _calculateFontSize(size, style, maxLines);
      final double fontSize = result[0];
      final bool textFits = result[1];
  ...
  List<dynamic> _calculateFontSize(BoxConstraints size, TextStyle style, int maxLines) {
    ...
    if (presetFontSizes == null) {
      ...
      final double defaultScale = defaultFontSize * userScale / style.fontSize;
      if (_checkTextFits(span, defaultScale, maxLines, size)) {
        return <dynamic>[defaultFontSize * userScale, true];
      }
    ...
    return <dynamic>[fontSize, lastValueFits];
  }
  ...
  bool _checkTextFits(
    ...
    if (!widget.wrapWords) {
      ...
      final TextPainter wordWrapTp = TextPainter(
        text: TextSpan(
          style: text.style,
          text: words.join('\n'),
        ),
