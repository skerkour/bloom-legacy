<template>
  ...
  <v-card>
    ...
    <v-card-text>
      ...
      :disabled="is_loading"
      @keyup.enter.native="rename_playlist"
      ></v-text-field>
    </v-card-text>
...
@Component
export default class CreatePlaylistDialog extends Vue {
  // props
  ...
  async rename_playlist() {
    ...
    try {
      const res = await api.put(`${api.MUSIC}/v1/playlists/${this.playlist.id}`, payload);
