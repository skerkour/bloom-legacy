...
pub fn config(config: Config) -> impl Fn(&mut web::ServiceConfig) {
    return move |cfg| {
        ...
        .service(if config.disabled.drive {
            web::scope("/drive").service(
                web::resource("*").route(web::route().to(kernel::api::route_disabled)),
            )
        } else {
            web::scope("/drive")
