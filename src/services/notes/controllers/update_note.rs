        ...
        return Ok(conn.transaction::<_, KernelError, _>(|| {
            ...
            let note_to_update = match msg.title {
                Some(title) => {
                    ...
                    let (note_to_update, event, _) = eventsourcing::execute(&conn, note_to_update, &update_title_cmd)?;
            ...
            let note_to_update = match msg.body {
                Some(body) => {
                    ...
                    let (note_to_update, event, _) = eventsourcing::execute(&conn, note_to_update, &update_body_cmd)?;
